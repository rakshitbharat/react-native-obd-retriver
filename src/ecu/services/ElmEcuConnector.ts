...
import { ECUConnectionStatus } from '../types'; // Keep this for now, might be needed elsewhere or implicitly
...
export const ELM_PROTOCOL_DESCRIPTIONS: { [key in ElmProtocols]: string } = { // Ensure export
  ...
  [ElmProtocols.ISO_15765_4_CAN_11B_500K]: 'ISO 15765-4 CAN (11 Bit ID, 500 KBit)',
  [ElmProtocols.ISO_15765_4_CAN_29B_500K]: 'ISO 15765-4 CAN (29 Bit ID, 500 KBit)',
  [ElmProtocols.ISO_15765_4_CAN_11B_250K]: 'ISO 15765-4 CAN (11 Bit ID, 250 KBit)',
  [ElmProtocols.ISO_15765_4_CAN_29B_250K]: 'ISO 15765-4 CAN (29 Bit ID, 250 KBit)',
  [ElmProtocols.SAE_J1939_CAN_29B_250K]: 'SAE J1939 CAN (29 bit ID, 250* kbaud)',
...
const _cleanResponse = (response: string | null): string => {
  ...
  return response.replace(/>$/, '').replace(/[\s\r\n\0]/g, '').toUpperCase().trim();
};

// --- The Class ---

export class ElmEcuConnector {
  private readonly sendCommand: (command: string, timeout?: number) => Promise<string | null>;
  private readonly log: (level: 'info' | 'warn' | 'error' | 'debug', message: string, data?: any) => void;
  ...
  private adaptiveTiming: AdaptiveTiming = { mode: 1, currentDelay: COMMAND_DELAY_MS };
  ...
  async connectToECU(): Promise<boolean> {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const initialResponse = await this._sendCommandInternal('\r', RESET_DELAY_MS); // Sending CR often wakes ELM
        if (initialResponse === null) {
          this.log('warn', 'No initial response from device. Trying ATZ.');
          const resetResponse = await this._sendCommandInternal(ELM_COMMANDS.RESET, RESET_DELAY_MS);
          if (resetResponse === null) {
              throw new Error('Device not responding to initial commands (CR, ATZ)');
          }
        }
        ...
        if (!await this._executeCommand(ELM_COMMANDS.RESET, RESET_DELAY_MS)) {
        ...
        if (detectedProtocol !== null) {
          ...
          this.log('info', `Protocol detected: ${this.getProtocolDescription()} (ID: ${this.currentProtocol})`);
        ...
        this.log('error', 'ECU connection process failed', { error: error.message, stack: error.stack });
  ...
  async disconnect(): Promise<void> {
      ...
      try {
          ...
          await this._sendCommandInternal(ELM_COMMANDS.PROTOCOL_CLOSE, COMMAND_DELAY_MS);
      } catch (error) {
          this.log('warn', 'Failed to send protocol close command during disconnect', error);
      } finally {
          this._resetState();
          this._updateStatus(ProtocolStatus.DISCONNECTED);
  ...
  private async _sendCommandInternal(command: string, delayAfterMs: number = COMMAND_DELAY_MS): Promise<string | null> {
    ...
    try {
        ...
        this.log('debug', `Received response: [${response?.trim() ?? 'null'}] (Cleaned: [${cleaned}])`);
        ...
        if (response === 'COMMAND_FAILED') {
             this.log('error', `Critical communication failure reported for command: ${command}`);
             this._updateStatus(ProtocolStatus.COMMAND_FAILED);
             throw new Error(`Critical communication failure on command: ${command}`); // Propagate critical failure
        }
        ...
        if (this.status !== ProtocolStatus.COMMAND_FAILED) {
            this.log('error', `Error sending command: ${command}`, { error: error.message });
        }
  ...
  private async _executeCommand(command: string, delayAfterMs: number = COMMAND_DELAY_MS): Promise<boolean> {
    ...
    if (cleaned.endsWith('OK')) {
      return true;
       ...
       this.log('warn', `Command ${command} received error response: ${response.trim()}`);
       return false;
    } else {
       ...
       this.log('warn', `Command ${command} received unexpected response: ${response.trim()}`);
  ...
  private async _initializeElm(): Promise<boolean> {
    ...
    for (const cmd of initCommands) {
      ...
      if (response === null) {
         this.log('warn', `No response during init for command: ${cmd}. Continuing...`);
         // Some ELM clones might not respond reliably to all init commands
         // Decide if this should be a hard failure.
         // return false;
      } else if (this._isErrorResponse(response)) {
         this.log('error', `Error response during init for command: ${cmd} -> ${response.trim()}`);
         return false;
      }
  ...
  private async _detectProtocol(): Promise<ElmProtocols | null> {
    this.log('info', 'Attempting automatic protocol detection (ATSP0 + 0100)...');
    ...
    if (this._isValidDataResponse(response0100)) {
      this.log('debug', `Received valid response for ${TEST_COMMAND_PID} on Auto protocol.`);
      // Now check which protocol was actually used
      const detected = await this._checkProtocolNumber();
      if (detected !== null) {
        this.log('info', `Auto-detection successful. Protocol identified as: ${detected}`);
         // Try to detect ECUs from the 0100 response
        await this._handleEcuDetection(response0100);
        return detected;
      } else {
         this.log('warn', `Command ${TEST_COMMAND_PID} succeeded, but failed to read protocol number (ATDPN).`);
      }
    } else {
    ...
    this.log('info', 'Auto-detection failed or inconclusive. Trying specific protocols...');
    for (const protocol of PROTOCOL_TRY_ORDER) {
      this.log('info', `Trying protocol: ${ELM_PROTOCOL_DESCRIPTIONS[protocol]} (ID: ${protocol})...`);
      ...
      if (this._isValidDataResponse(testResponse)) {
        ...
        return protocol;
          ...
          this.log('debug', `Protocol ${protocol} failed test command ${TEST_COMMAND_PID}.`);
    ...
    this.log('error', 'Failed to detect any working protocol after trying all options.');
    return null;
  }

  /** Sends ATDPN to get the current protocol number */
  private async _checkProtocolNumber(): Promise<ElmProtocols | null> {
    const response = await this._sendCommandInternal(ELM_COMMANDS.GET_PROTOCOL_NUM);
    const cleaned = _cleanResponse(response);
    ...
    if (cleaned) {
      ...
      if (match && match[1]) {
          ...
          if (protocolId === 'A') { protocolId = 10; }
          else if (protocolId === 'B') { protocolId = 11; }
          else if (protocolId === 'C') { protocolId = 12; }
          else { protocolId = parseInt(protocolId, 10); }
    ...
    this.log('warn', `Could not parse protocol number from ATDPN response: ${response?.trim() ?? 'null'}`);
    return null;
  }
  ...
  private async _handleEcuDetection(response: string | null): Promise<void> {
    ...
    if (detected.size > 0) {
      ...
      if (this.selectedEcuAddress) {
        await this.log('info', `ECU address ${this.selectedEcuAddress} selected for communication.`);
        this._updateStatus(ProtocolStatus.ECU_DETECTED);
      } else {
        this.log('warn', 'Detected ECU headers but could not select a primary address.');
      }
    } else {
      this.log('warn', 'Valid data response received, but no ECU headers detected.');
    }
  }
}
