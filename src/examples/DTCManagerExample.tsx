...
export const DTCManagerExample: React.FC = (): JSX.Element => {
  ...
  const [permanentDTCs, setPermanentDTCs] = useState<RawDTCResponse | null>(null);
  ...
  const fetchCurrentDTCs = useCallback(async () => {
    ...
    try {
      ...
      setLastError(`Current DTCs Error: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
      setLoading(null);
    }
  }, [get03DTCObject]);
  ...
  const fetchPendingDTCs = useCallback(async () => {
    ...
    try {
      ...
      setLastError(`Pending DTCs Error: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
      setLoading(null);
    }
  }, [get07DTCObject]);
  ...
  const fetchPermanentDTCs = useCallback(async () => {
    ...
    try {
      ...
      setLastError(`Permanent DTCs Error: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
      setLoading(null);
    }
  }, [get0ADTCObject]);
  ...
  const fetchAllDTCs = useCallback(async () => {
    ...
    try {
      ...
      setLastError(`All DTCs Error: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
      setLoading(null);
    }
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.connectionButtons}>
        <Button
          ...
          disabled={state.status === ECUConnectionStatus.CONNECTING || isLoading}
        />
      </View>
